...
import {PacketPubcomp} from '../pubcomp';
import {MqttDecoder} from '../../MqttDecoder';

test('can create a packet', () => {
  const packet = PacketPubcomp.create(0, 0, {});
  expect(packet.b >> 4).toBe(PACKET_TYPE.PUBCOMP);
...
test('can serialize basic packet MQTT 5.0', () => {
  const packet = PacketPubcomp.create(0, 0, {});
  expect(packet instanceof PacketPubcomp).toBe(true);
  const buf = packet.toBuffer(5);
  expect(buf).toEqual(Buffer.from([
    PACKET_TYPE.PUBCOMP << 4,   // Header
...
test('can serialize packet and deserialize it back', () => {
  const packet1 = PacketPubcomp.create(1, 2, {});
  const decoder = new MqttDecoder();
  decoder.version = 5;
  decoder.push(packet1.toBuffer(5));
  const packet2 = decoder.parse()! as PacketPubcomp;
