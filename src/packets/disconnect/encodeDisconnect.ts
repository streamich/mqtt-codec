import {PacketDisconnectData} from '.';
import {PACKET_TYPE} from '../../enums';
import {genProps} from '../../util/genProps';

export const DISCONNECT = Buffer.from([PACKET_TYPE.DISCONNECT << 4, 0]);

export const encodeDisconnect = (packet: PacketDisconnectData, version: number): Buffer => {
  const isV5 = version === 5;
  if (!isV5) return DISCONNECT;

  const {b, c, p} = packet;
  const props = genProps(p);
  const propsLength = props.length;
  const remainingLength: number = 1 + (propsLength > 1 ? propsLength : 0);
  ...
  if (propsLength > 1) props!.copy(buf, offset + 1);
